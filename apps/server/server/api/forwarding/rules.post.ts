...
const Rule = z.object({ destinationId: Id, actionKind: z.string().min(1).max(100), includeAction: z.boolean().default(false), includeArtefactMetadata: z.boolean().default(false) });
export default defineEventHandler(async (event) => {
  ...
  const [row] = await database().sql`insert into forwarding_rules (id, workspace_id, destination_id, action_kind, include_action, include_artefact_metadata) values (${createId()}, ${auth.workspaceId}, ${input.destinationId}, ${input.actionKind}, ${input.includeAction}, ${input.includeArtefactMetadata}) returning id`;
