...
export default defineEventHandler(async (event) => {
  ...
  const reservation = await database().sql.begin(async (sql) => {
    ...
    `;
    if (existing) {
      if (existing.state === "DELETING" || existing.unexpired !== true) {
        throw createError({ statusCode: 409, statusMessage: "The staged artefact upload has expired" });
      }
  ...
  const store = objects();
  const result = await store.putIfAbsent(objectKey, body, mediaType);
  if (result === "exists") {
  ...
  await database().sql.begin(async (sql) => {
    const [current] = await sql`
      select state, expires_at > now() as unexpired from artefacts
      where id = ${reservation.id} and upload_payload_hash = ${payloadHash}
      for update
    `;
    if (!current || current.state === "DELETING" || current.unexpired !== true) {
      throw createError({ statusCode: 409, statusMessage: "The staged artefact upload has expired" });
    }
    if (current.state === "UPLOADING") {
      ...
      throw createError({ statusCode: 409, statusMessage: "The staged artefact upload is unavailable" });
    }
  });
