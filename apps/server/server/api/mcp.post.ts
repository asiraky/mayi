import { CreateApproval, canonicalDigest } from "@mayi/contracts";
...
const tools = [
  { name: "create_approval", description: "Create and seal an exact approval request without evidence. Use HTTP uploads for evidence.", inputSchema: { type: "object", required: ["action", "explanation", "idempotencyKey"], properties: { action: { type: "object" }, explanation: { type: "string" }, expiresInSeconds: { type: "integer" }, enforcement: { enum: ["cooperative", "verified", "consumed"] }, suggestedApproverId: { type: "string", format: "uuid" }, idempotencyKey: { type: "string" } } } },
  { name: "get_approval", description: "Read authoritative approval state and receipt.", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string", format: "uuid" } } } },
  { name: "cancel_approval", description: "Cancel an owned draft or pending approval.", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string", format: "uuid" } } } },
];
  ...
  if (name === "create_approval") {
    ...
    const id = await database().sql.begin("isolation level serializable", async (sql) => {
      ...
      const [approval] = await sql`
        insert into approvals (workspace_id, agent_id, state, action, explanation, enforcement, action_digest, manifest_digest, policy_version, high_risk, expires_at, sealed_at)
        values (${auth.workspaceId}, ${auth.agentId}, 'PENDING', ${JSON.stringify(input.action)}::jsonb, ${input.explanation}, ${input.enforcement}, ${digests.actionDigest}, ${digests.manifestDigest}, ${workspace!.policy_version}, ${isHighRisk(input.action)}, now() + make_interval(secs => ${input.expiresInSeconds}), now()) returning id
      `;
      ...
      await sql`insert into idempotency_keys (workspace_id, credential_id, operation, key, payload_hash, response, expires_at) values (${auth.workspaceId}, ${auth.agentId}, 'approval.create', ${idempotencyKey}, ${payloadHash}, ${JSON.stringify({ id: approvalId })}::jsonb, now() + interval '24 hours')`;
      await sql`insert into jobs (workspace_id, type, dedupe_key, payload) values (${auth.workspaceId}, 'push.approval_pending', ${approvalId}, ${JSON.stringify({ approvalId })}::jsonb) on conflict do nothing`;
      ...
      for (const rule of rules) {
        const deliveries = await sql`insert into forwarding_deliveries (workspace_id, approval_id, destination_id, origin_id) values (${auth.workspaceId}, ${approvalId}, ${rule.destination_id}, ${approvalId}) on conflict do nothing returning id`;
        if (deliveries[0]) await sql`insert into jobs (workspace_id, type, dedupe_key, payload) values (${auth.workspaceId}, ${rule.type === "EMAIL" ? "email.approval_pending" : "webhook.approval_pending"}, ${`${approvalId}:${rule.destination_id}`}, ${JSON.stringify({ approvalId, destinationId: String(rule.destination_id), deliveryId: String(deliveries[0].id) })}::jsonb) on conflict do nothing`;
      }
